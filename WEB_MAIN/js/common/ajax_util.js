 ...
 * @param param   	콜백함수에서 사용되는 파라메터
 * @param data	   	송신하는 데이터 (&이름1=값1&이름2=값2...)
 * @param url      	요청하는 파일의 URL
 */
function ajaxSendPost(callback, param, data, url){
	sendRequest(callback, param, data, 'POST', url, false)
}
...
//
function sendRequest(callback, param, data, method, url, async, sload, user, password)
{
	//XMLHttpRequest 오브젝트 생성
