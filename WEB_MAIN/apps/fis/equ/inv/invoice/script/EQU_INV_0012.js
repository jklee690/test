...
function doWork(srcName){
    ...
    try {
        ...
        case "SEARCHLIST":
			...
			sheetObj.DoSearch4Post("EQU_INV_0012GS.clt", FormQueryString(formObj));
        ...
        case "ROWADD":
           ...
           if((sheetObj.RowCount)>1){ // no를 설정한다.
        	   	sheetObj.CellValue2(Row, "no") = parseInt(sheetObj.CellValue(sheetObj.LastRow-2, "no"))+1;
        ...
        case "ADD":
        	   ...
        	   if(objArr[2]!=''){
        		   isOk = false;
        		   alert(getLabel('EQU_INV_MSG25'));
        	   }

        	   if(isOk){

					if(confirm(getLabel('EQU_INV_MSG24'))){
...
//						
						...
						sheetObj.DoSave("EQU_INV_0012GS.clt", FormQueryString(formObj),"ibflag",false);
//	                	docObjects[1].DoAllSave("./EQU_INV_0012GS.clt", FormQueryString(frm1)+docObjects[0].GetSaveString(false), true);
					}        	    	
        	   }
           break;
        case "APPLY":
        	...
        	strHtml = strHtml.replace(/<TABLE/gi, "<TABLE  ID='input' style='vertical-align:top;font-size:10pt;font-family:Tahoma;' border='0' cellspacing='0' cellpadding='0'" +
        			"width='1300' align='left' valign='middle'>thisinput</TABLE1>" +
        			"\n<TABLE");
        	strHtml = strHtml.replace('thisinput', snd_msg);
        	strHtml = strHtml.replace('1170', "1300");
         ...
         case 1:      //IBSheet1 init
            with (sheetObj) {
               ...
               //전체Edit 허용 여부 [선택, Default false]
                ...
                InitHeadRow(0, getLabel('EQU_INV_0012_HDR'), false);
                
                //데이터속성    [ROW,   COL,   DATATYPE,  WIDTH, DATAALIGN, COLMERGE, SAVENAME,    KEYFIELD, CALCULOGIC, DATAFORMAT, POINTCOUNT, UPDATEEDIT, INSERTEDIT, EDITLEN, FULLINPUT, SORTENABLE, TOOLTIP, ALLCHECK, SAVESTATUS, FORMATFIX]
 				InitDataProperty(0, cnt++,  dtHiddenStatus,	 40,    daCenter,  true,    "dtl_ibflag");
 				InitDataProperty(0, cnt++,  dtDelCheck,		 30,    daCenter,  true,    "chk",			false,	"",       dfNone,      0,      true,	true,   1, false, false, "", false);
 				InitDataProperty(0, cnt++,  dtData,		 	 50,    daCenter,  true,    "no", 			false,	"",       dfNone,      0,     false,	false,	3);
 				InitDataProperty(0, cnt++,  dtData,			100,    daLeft,    false,   "bkg_no",		false,	"",       dfNone,      0,     false,	true,	6);
 				InitDataProperty(0, cnt++,  dtData,			 90,    daLeft,    false,   "lr_trdp_nm",	false,	"",       dfNone,      0,     false,	true,	50);
 				InitDataProperty(0, cnt++,  dtData,			 50,    daCenter,  false,   "cntr_tpsz_cd",	false,	"",       dfNone,      0,     false,	true,	6);
 				InitDataProperty(0, cnt++,  dtData,			110,    daRight,   true,    "unit_prc",		false,	"",       dfFloat,     2,     false,	true,	20);
 				InitDataProperty(0, cnt++,  dtData,			 40,    daRight,   true,    "qty", 			false,	"",       dfInteger,   0,     false,	true,	7);
 				InitDataProperty(0, cnt++,  dtAutoSum,		110,    daRight,   true,    "amt",			false,	"",       dfFloat,     2,     false,	true,	20);
 				InitDataProperty(0, cnt++,  dtAutoSum,		110,    daRight,   true,    "inv_vat_amt",	false,	"",       dfFloat,     2,     false,	true,	20);
 				InitDataProperty(0, cnt++,  dtData,			100,    daLeft,    true,    "pkup_nod_nm",	false,	"",       dfNone,      2,     false,	true,	50);
 				InitDataProperty(0, cnt++,  dtData,			100,    daLeft,    true,    "cy_nm",		false,	"",       dfNone,      2,     false,	true,	50);
 				InitDataProperty(0, cnt++,  dtData,			100,    daLeft,    true,    "cy_pkup_no",	false,	"",       dfNone,      2,     false,	true,	50);
				...
				InitDataProperty(0, cnt++,  dtData,			 80,    daLeft,    true,    "splr_tel_no",	false,	"",       dfNone,      0,     false,	true,	20);
 				InitDataProperty(0, cnt++,  dtHidden,		 70,    daLeft,    true,    "inv_no", 		false,	"",       dfNone,      0,     false,	true,	14);
 				InitDataProperty(0, cnt++,  dtHidden,		 70,    daLeft,    true,    "xch_rt", 		false,	"",       dfNone,      0,     false,	true,	14);
 				InitDataProperty(0, cnt++,  dtHidden,		 70,    daLeft,    true,    "vat_rt", 		false,	"",       dfNone,      0,     false,	true,	14);
 				InitDataProperty(0, cnt++,  dtHidden,		 70,    daLeft,    true,    "tot_bkg_no", 		false,	"",       dfNone,      0,     false,	true,	14);
...
function sheet1_OnSearchEnd(){
	...
	formObj.eml_tit.value = docObjects[0].CellValue(1, 'tot_bkg_no');
}
