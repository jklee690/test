    ...
    switch(srcName) {
		case "SEARCHLIST":
            ...
            if(validateForm(sheetObj, formObj, SEARCHLIST, 1)){
            	sheetObj.DoSearch4Post("MDM_MCM_0010GS.clt", FormQueryString(formObj));
            }
		...
		case "MODIFY":
			//좌측 목록에서 선택 하지 않고 수정한 경우
			if(frm1.i_rgst_usrid.value==''){
				//Same continent code is found in the [Continent List]!\n\nDo you want to update [Continent Code]?
				alert(getLabel('FMS_COM_ALT008') + "\n\n: MDM_MCM_0010.34");
				
			}else if(confirm(getLabel('FMS_COM_CFMMOD'))){
				useFlgChange();
		...
		case "REMOVE":
            ...
            if(validateForm(sheetObj,formObj,REMOVE, 1)){
                if(confirm(getLabel('FMS_COM_CFMDEL'))){
                    doProcess = true;
                    sheetObj.DoSave("MDM_MCM_0010GS.clt", FormQueryString(formObj),"ibflag",false);
                }
			}
         ...
         case 1:      //IBSheet1 init
            ...
            with (sheetObj) {
               ...
               //전체Edit 허용 여부 [선택, Default false]
                ...
                InitHeadRow(0, getLabel('MDM_MCM_0010_HDR'), true);
                ...
                InitDataProperty(0, 1,  dtHidden,      	  40,   daCenter,  true,    "ibflag");
      			...
      			InitDataProperty(0, 4,  dtData,           80,   daLeft,    true,    "locl_nm",   false,      "",       dfNone,      0,     false,      false);
      			InitDataProperty(0, 5,  dtData,          220,   daLeft,    true,    "desc",   false,      "",       dfNone,      0,     false,      false);
      			...
      			InitDataProperty(0, 9,  dtHidden,         80,   daLeft,    true,    "rgst_usrid",   false,      "",       dfNone,      0,     false,      false);
      			InitDataProperty(0,10,  dtHidden,         80,   daLeft,    true,    "rgst_ofc_cd",   false,      "",       dfNone,      0,     false,      false);
      			InitDataProperty(0,11,  dtHidden,         80,   daLeft,    true,    "rgst_tms",   false,      "",       dfNone,      0,     false,      false);
      			InitDataProperty(0,12,  dtHidden,         80,   daLeft,    true,    "modi_usrid",   false,      "",       dfNone,      0,     false,      false);
      			InitDataProperty(0,13,  dtHidden,         80,   daLeft,    true,    "modi_ofc_cd",   false,      "",       dfNone,      0,     false,      false);
      			InitDataProperty(0,14,  dtHidden,         80,   daLeft,    true,    "modi_tms",   false,      "",       dfNone,      0,     false,      false);
      			InitDataProperty(0,15,  dtHidden,          0,   daCenter,  false,    "Indexing");
           }                                                      
           break;
     }
}
...
function sheet1_OnClick(sheetObj,Row,Col){
	...
	formObj.i_rgst_usrid.value = sheetObj.CellValue(Row, 9);
	formObj.i_rgst_tms.value = sheetObj.CellValue(Row, 11);
	formObj.i_modi_usrid.value = sheetObj.CellValue(Row, 12);
	formObj.i_modi_tms.value = sheetObj.CellValue(Row, 14);
...
function checkAddModiVal(frm1){
	var formObj  = document.frm1;
    if(checkInputVal(formObj.i_conti_cd.value, 2, 2, "T", getLabel('CONTI_CD'))!='O'){
    	return false;
    } else if(checkInputVal(formObj.i_locl_nm.value, 1, 100, "T", getLabel('LOCAL_NM'))!='O'){
    	return false;
    } else if(checkInputVal(formObj.i_eng_nm.value, 1, 50, "T", getLabel('ENG_NM'))!='O'){
    	return false;
