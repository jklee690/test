...
function doWork(srcName){
    ...
    switch(srcName) {
      ...
      break;
      case 'PRINT':
    	  formObj.title.value='Total Profit Report (A)';
          var param="";
          ...
          var endDt_ymd=mkFormat1(formObj.s_prd_enddt.value);
      	  // ==============  rate 를 query 로   ==============
		  rateSql=getRateQuery();
          param += '[' + curr_opt + ']';					//$1	curr_opt
		  ...
		  //One 인 경우
		  param += '[' + formObj.s_curr_cd.value + ']';	    //$8   currency
		  param += '[' + rateSql + ']';				    	//$9   Currency SQL
		  param += '[' + formObj.f_ofc_locl_nm.value + ']';   //$10 
		  if(curr_opt == "M")
		  {
			  file_names='pfm_profit_total_multi_A.mrd';
		  }
		  if(curr_opt == "O")
		  {
			  file_names='pfm_profit_total_one_A.mrd';
		  }
...
function initSheet(sheetObj,sheetNo) {
    switch(sheetNo) {
        ...
        case 2:      //IBSheet2 init
	          ...
	          var info    = { Sort:1, ColMove:1, HeaderCheck:1, ColResize:1 };
	          var headers = [ { Text:"MultiCurr.", Align:"Center"} ];
	          InitHeaders(headers, info);
...
function getRadioVal(radioObj){
   ...
   if(curr_opt == "M"){
        ...
        //sheetObj2.DoSearch4Post("./PFM_MGT_0060_2GS.clt", FormQueryString(formObj));
   }else if(curr_opt == "O"){
