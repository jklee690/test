function doWork(srcName){
    ...
    switch(srcName) {
    	...
    	case "SEARCH01": 
            ...
            formObj.action = "./SEI_DOC_0032.clt";
            formObj.submit();
       	break;
       	case "SEARCH02":
        ...
        case "DOSEARCH":
       	...
       	case "ISSUE":
			...
			formObj.palt_doc_tp_cd.value = "DO";	
...
function doDOAction(){
	var formObj  = document.frm1;
	var s_bztp_cd = "DO";
	ajaxSendPost(dispDOAjaxReq, 'reqVal', '&goWhere=aj&bcKey=searchANDO&s_bztp_cd='+s_bztp_cd, './GateServlet.gsl');
}

//ANDO combo표시 Ajax
function dispDOAjaxReq(reqVal){
	...
	if(doc[0]=='OK'){
		if(typeof(doc[1])!='undefined'){
			...
			for(var i = 0; i<arrLen; i++){
				...
				frm1.s_tmplt_tit.options[loopCnt]=new Option(masterVals[2],masterVals[0]);
				loopCnt++;
			}
			/*
			...
			if(s_tmplt_seq !=""){
				...
				frm1.s_tmplt_tit.options[tmplt_seq].selected = true;
			}
			*/					
		}
		else{
			frm1.s_tmplt_tit.options[0]     = new Option( '',  ''  );
			frm1.s_tmplt_tit.options.length = '1';
	
			formObj.tmplt_tit.value   = "";
			formObj.palt_doc_msg.value= "";
		}
	}
	else{
...
function dispTitleAjaxReq(reqVal){
	...
	if(doc[0]=='OK'){
		if(typeof(doc[1])!='undefined'){
			...
			for(var i = 0; i<arrLen; i++){
				var masterVals = rtnArr[i].split(',');	
				frm1.palt_doc_nm.value  = masterVals[2];
				frm1.palt_doc_msg.value = masterVals[3];
			}
		}
	}
	else{
